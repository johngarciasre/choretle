...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    const { name, description, points, icon, tags } = body;
    const taskId = (await params).taskId;
    ...
    // Update task
    const updates: string[] = ["name = ?", "description = ?", "points = ?", "icon = ?", "updated_at = ?"];
    const values: any[] = [name || "", description || null, points || 0, icon || null, new Date().toISOString()];
    values.push(taskId);
    ...
    return NextResponse.json({
      ...
      points: updatedTaskWithTags.points || 0, icon: updatedTaskWithTags.icon, tags: (tagDetails || []).map((t: any) => ({ id: t.id, name: t.name, color: t.color })),
    });
  } catch (err) {
